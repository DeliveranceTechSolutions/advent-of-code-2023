...
func main() {
	data := loadData()
	gameSheet := &GameSheet{}
	elf := Game{ Red: 12, Blue: 14, Green: 13 }
	total := 0
	for _, game := range data {
		loggedGame := analyzeGame(strings.Split(game, " "), &elf)
		gameSheet.Logs = append(gameSheet.Logs, *loggedGame)
	}

	for _, game := range gameSheet.Logs {
		possible := true
		for _, outcome := range game.Outcomes {
			if outcome.Red > elf.Red ||
			outcome.Blue > elf.Blue	||
			outcome.Green > elf.Green {
				possible = false
				break
			}
		}
	...
	fmt.Println(gameSheet)

	fmt.Println(total)
}

func analyzeGame(game []string, elf *Game) *GameLog {
	gameID, err := strconv.Atoi(game[1][:len(game[1]) - 1]);
	if err != nil {
		panic(err)
	}
	...
	for score, word := range game {
		...
		switch word {
		case "red":
			num := atoi(game[score - 1])
			if (elf.Red > num) {

			}
			currentGame.Red = atoi(game[score - 1])
		case "blue":
			currentGame.Blue = atoi(game[score - 1])
		case "green":
			currentGame.Green = atoi(game[score - 1])
		}

		if endOfCycle {
			analysis.Outcomes = append(analysis.Outcomes, currentGame)
			currentGame = Game{}
			endOfCycle = !endOfCycle
		}
	}
