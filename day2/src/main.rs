...
fn main() {
	let data:Vec<&str>  = load_data();
	let sheet: GameSheet = GameSheet{ logs: vec![] };
...
fn parse_data(row: &'static str) {
	let outcomes = row.split(" ");
	let game = Game{ red: 0, blue: 0, green: 0 };
	let log = Logs{ id: 0, games: vec![] };
	let prev = "";
	for outcome in outcomes {
		let last = outcome.chars().last().unwrap();

		match last {
			':' => log.id = last.to_digit(10).unwrap(),
			',' => ,
			_ => None,
		};
		println!("{:?}", outcome);
